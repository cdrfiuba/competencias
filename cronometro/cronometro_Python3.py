...
SERIAL_PORT =  "COM5" # "/dev/ttyACM0" o "COM3" "/dev/ttyUSB2"
...
class SerialIO(object):
    ...
    def try_read_byte(self):
        rcv = self.ser.read(1).decode('utf-8')
        return rcv
...
def imprimir_carreras(carreras, archivo):
    carreras.sort()
    for n, carrera in list(enumerate(carreras)):
        print( n+1, carrera)
    ...
    for event in pygame.event.get():
        if event.type == pygame.QUIT:
            ...
            if event.key == pygame.K_ESCAPE:
                ...
                for n, texto in list(enumerate(textos)):
